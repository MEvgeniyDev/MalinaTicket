...
MalinaTicket - простой тикет-плагин для Paper/Purpur `26.1.2`. Он закрывает обычный сценарий небольшого сервера: игрок оставляет обращение через GUI, персонал отвечает, назначает ответственного и закрывает тикет с причиной.
...
Плагин не заявляет совместимость со Spigot, Folia и старыми ветками Paper. Если нужен перенос на другую версию сервера, сначала собери отдельный тестовый стенд и проверь GUI/чатовые события.
